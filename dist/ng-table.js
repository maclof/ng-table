    ...
    angular.module('ngTable').factory('ngTableColumn', [function () {
        ...
        function createDefaults(){
            return {
                ...
                heading: createGetterSetter(''),
                headingAlt: createGetterSetter('')
            };
        }
            ...
            return {
                ...
                compile: function(element) {
                    ...
                    angular.forEach(dataRow.find('td'), function(item) {
                        ...
                        var headingExpr = getAttrValue('heading-alt') || getAttrValue('heading');
                        if (headingExpr){
                            el.attr('data-heading-text', '{{' + headingExpr + '}}'); // this used in responsive table
                        }
                        // NOTE TO MAINTAINERS: if you add extra fields to a $column be sure to extend ngTableColumn with
                        // a corresponding "safe" default
                        columns.push({
                            id: i++,
                            heading: parsedAttribute('heading'),
                            headingAlt: parsedAttribute('heading-alt'),
                            headerTitle: parsedAttribute('header-heading'),
        ...
        return {
            ...
            compile: function(tElement) {
                ...
                angular.forEach(row.find('td'), function(item) {
                    ...
                    var headingExpr = getAttrValue('heading');
                    if (!headingExpr){
                        el.attr('data-heading-text', '{{$columns[$index].headingAlt(this) || $columns[$index].heading(this)}}');
                    }
...
(function(){
    ...
    function ngTableGroupRowController($scope){
        ...
        function getGroupTitle(group){
            return angular.isFunction(group) ? group.heading : group.heading($scope);
        }
...
angular.module('ngTable').run(['$templateCache', function ($templateCache) {
	$templateCache.put('ng-table/filterRow.html', '<tr ng-show="show_filter" class="ng-table-filters"> <th data-heading-text="{{$column.headingAlt(this) || $column.heading(this)}}" ng-repeat="$column in $columns" ng-if="$column.show(this)" class="filter {{$column.class(this)}}" ng-class="params.settings().filterOptions.filterLayout===\'horizontal\' ? \'filter-horizontal\' : \'\'"> <div ng-repeat="(name, filter) in $column.filter(this)" ng-include="config.getTemplateUrl(filter)" class="filter-cell" ng-class="[getFilterCellCss($column.filter(this), params.settings().filterOptions.filterLayout), $last ? \'last\' : \'\']"> </div> </th> </tr> ');
	...
	$templateCache.put('ng-table/filters/select-multiple.html', '<select ng-options="data.id as data.heading for data in $column.data" ng-disabled="$filterRow.disabled" multiple ng-multiple="true" ng-model="params.filter()[name]" class="filter filter-select-multiple form-control" name="{{name}}"> </select> ');
	$templateCache.put('ng-table/filters/select.html', '<select ng-options="data.id as data.heading for data in $selectData" ng-table-select-filter-ds="$column" ng-disabled="$filterRow.disabled" ng-model="params.filter()[name]" class="filter filter-select form-control" name="{{name}}"> <option style="display:none" value=""></option> </select> ');
	...
	$templateCache.put('ng-table/sorterRow.html', '<tr class="ng-table-sort-header"> <th heading="{{$column.headerTitle(this)}}" ng-repeat="$column in $columns" ng-class="{ \'sortable\': $column.sortable(this), \'sort-asc\': params.sorting()[$column.sortable(this)]==\'asc\', \'sort-desc\': params.sorting()[$column.sortable(this)]==\'desc\' }" ng-click="sortBy($column, $event)" ng-if="$column.show(this)" ng-init="template=$column.headerTemplateURL(this)" class="header {{$column.class(this)}}"> <div ng-if="!template" class="ng-table-header" ng-class="{\'sort-indicator\': params.settings().sortingIndicator==\'div\'}"> <span ng-bind="$column.heading(this)" ng-class="{\'sort-indicator\': params.settings().sortingIndicator==\'span\'}"></span> </div> <div ng-if="template" ng-include="template"></div> </th> </tr> ');
}]);
